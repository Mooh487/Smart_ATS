...
import { Download, Share2, RotateCcw, Mail, Save } from "lucide-react";
...
export const ResultsActions: React.FC<ResultsActionsProps> = ({
  ...
  const { saveReview } = useAnalysisStore();
  ...
  const handleSaveReview = async () => {
    if (!isAuthenticated) {
      toast.error("Please log in to save your review");
      return;
    }
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {isAuthenticated && (
          <button
            ...
            disabled={isSaving}
            className="flex flex-col items-center space-y-2 p-4 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
